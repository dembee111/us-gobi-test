...
class ProductRecent extends Component {
  render() {
    ...
    if (this.props.recentlyGrid && this.props.recentlyGrid.current && this.props.recentlyGrid.current.offsetWidth) {
      if (window.innerWidth >= 1024) {
        singleGridWidth = this.props.recentlyGrid.current.offsetWidth / 4 - 4;
      }
      if (window.innerWidth <= 1023) {
        singleGridWidth = this.props.recentlyGrid.current.offsetWidth / 3;
      }
      if (window.innerWidth < 769) {
        singleGridWidth = this.props.recentlyGrid.current.offsetWidth / 2;
      }
    ...
    const productsRecommendations = this.props.recentlyItemList.map((item, index) => {
    ...
    return (
      <div className="tc_recently_products" ref={this.props.recentlyGrid}>
        <Swiper
          swiperOptions={{
            slidesPerView: 4,
            spaceBetween: 4,
        ...
        <div className="swiper-button-prev-recently"></div>
        <div className="swiper-button-next-recently"></div>
      </div>
    );
  }
}

export default React.memo(ProductRecent);
